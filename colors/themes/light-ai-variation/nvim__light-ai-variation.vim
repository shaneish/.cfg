" Name:   active_theme
" Author: (me)

hi clear
if exists("syntax_on")
  syntax reset
endif

let g:colors_name = "active_theme"
set termguicolors
set background=light

" Editor UI and Syntax Highlighting
"
" group             guifg                                 guibg                                  gui
hi Normal           guifg=#000000                  guibg=#ffffff                   gui=NONE
hi Comment          guifg=#484848                                               gui=italic
hi Constant         guifg=#2e2e2e                                               gui=NONE
hi String           guifg=#6b876b                                           gui=NONE
hi Identifier       guifg=#103310                                           gui=NONE
hi Function         guifg=#000000                                                         gui=bold
hi Statement        guifg=#2a4c2a                                           gui=bold
hi PreProc          guifg=#2a4c2a                                           gui=NONE
hi Type             guifg=#103310                                           gui=NONE
hi Special          guifg=#6b876b                                           gui=NONE
hi Operator         guifg=#6b876b                                           gui=NONE
hi Todo                                                   guibg=#dce8dc     gui=bold,underline
hi Error            guifg=#e67373                                                      gui=bold,undercurl
hi ErrorMsg         guifg=#e67373                                                      gui=undercurl
hi WarningMsg       guifg=#2e2e2e        guibg=#ffe0e0          gui=NONE

" UI Elements
hi Cursor           guifg=#000000                  guibg=#b0ffb0
hi CursorLine                                             guibg=#f0fff0
hi CursorLineNr     guifg=#2a4c2a                                           gui=bold
hi ColorColumn                                            guibg=#f0fff0
hi Visual           guifg=#000000                  guibg=#e6e8c8
hi CursorColumn     guifg=NONE                            guibg=NONE                             gui=NONE
hi LineNr           guifg=#d0d0d0
hi NonText          guifg=#d0d0d0
hi VertSplit        guifg=#d0d0d0        guibg=#484848
hi StatusLine       guifg=#2a4c2a    guibg=#ededed         gui=bold
hi StatusLineNC     guifg=#484848        guibg=#d0d0d0         gui=NONE
hi Pmenu            guifg=#2e2e2e        guibg=#d0d0d0
hi PmenuSel         guifg=#000000                  guibg=#e6e8c8
hi PmenuSbar                                              guibg=#d0d0d0
hi PmenuThumb       guifg=#6b876b    guibg=#6b876b
hi Search                                                 guibg=#e6e8c8
hi IncSearch        guifg=#000000                  guibg=#e6e8c8
hi MatchParen       guifg=#000000                  guibg=#dce8dc     gui=bold

" Diffs
hi DiffAdd                                                guibg=#dce8dc
hi DiffDelete                                             guibg=#ffe0e0
hi DiffChange                                             guibg=#f0fff0
hi DiffText         guifg=#000000                  guibg=#e6e8c8            gui=bold

" Linking common groups for consistency
hi! link Number Constant
hi! link Boolean Constant
hi! link Float Constant
hi! link Character Constant
hi! link Keyword Statement
hi! link Conditional Statement
hi! link Repeat Statement
hi! link Label Statement
hi! link Exception Statement
hi! link Define PreProc
hi! link Include PreProc
hi! link Macro PreProc
hi! link PreCondit PreProc
hi! link StorageClass Type
hi! link Structure Type
hi! link Typedef Type
hi! link Title Function
hi! link StatusLine CursorLine

" Plugin-specific highlights
hi EyelinerPrimary      guifg=#000000           gui=bold
hi EyelinerSecondary    guifg=#484848 gui=bold,italic

