...
root_pane_id=$(wezterm cli spawn --domain-name local --cwd "$WORKSPACE_DIR" -- "$SHELL")
...
yml_pane_id=$(wezterm cli spawn --domain-name local --cwd "$WORKSPACE_DIR" -- $EDITOR $yml_files)
...
src_pane_id=$(wezterm cli spawn --domain-name local --cwd "$WORKSPACE_DIR/src" -- $EDITOR $src_files)
...
lib_pane_id=$(wezterm cli spawn --domain-name local --cwd "$WORKSPACE_DIR/src/ap_databricks" -- $EDITOR $lib_files)
...
py_scripts_pane_id=$(wezterm cli spawn --domain-name local --cwd "$WORKSPACE_DIR/scripts" -- $EDITOR $py_script_files)
...
scripts_pane_id=$(wezterm cli spawn --domain-name local --cwd "$WORKSPACE_DIR" -- $EDITOR $scripts_files)
